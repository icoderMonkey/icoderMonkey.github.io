...
在 **TypeScript** 中为变量添加类型约束，只需在变量的后面加上 **:type** 即可。

假设我要声明一个名为 **userName** 变量并设置为 **字符串类型**，如：

```TypeScript
let userName:string = "CoderMonkey"
```

被 **string** 限制后的 **userName**，意味着在赋值时变量值的类型只能为 **string** 或者为 **string 的子集**（**子集** 后面会详解），否则编译就会报错。
...
当设置为 **any** 类型时，可以说已经失去了对该变量值类型的检查，只要编写的代码语句没有明显的语法错误，就会被视为正确的逻辑；此时的 **TypeScript** 俨然已经变成了 **AnyScript**，也就间接地等于你是在编写 **JavaScript** 了。
...
未知类型，可以理解为 “严格版的 any”，其主要作用就是为了解决 any 会产生的类型污染问题。
...
这种情况下，可以使用 **typeof** 操作符来对变量的类型进行“缩小”，当明确该值的类型为 **string** 时，再去调用 **stirng** 本身的属性和方法那就不会有任何问题了，这一招（“类型缩小”）非常好使。
...
场景 :one:：函数 _throwError_ 会在内部抛出一个错误，这意味着该函数永远也不会有返回结果
...
场景 :two:：函数 _keepProcessing_ 内部 _while_ 循环条件为 **true**，这意味着该循环会一直执行下去，永远也不会停止，也就永远不会返回结果
...
上面两个函数的共同特点就是内部执行永远走不到头，因此永远也不会有一个明确的返回结果，所以，适合使用 **never** 类型。
...
变量 _userName_ 的类型为值类型 `CoderMonkey`，这意味着 _userName_ 的值只能为 `CoderMonkey`，不能是其他类型的值。

这种情况就等同于通过 _const_ 声明了常量 _userName_，并且值为 `CoderMonkey`
...
变量 _flag_ 的类型可以用 **boolean** 来限制，也可以用 **0** 或 **1** 来限制，所以就可以使用 “联合类型”，只要值的类型符合其中某个类型即可。
...
变量 _fullName_ 的类型实际上是一个 **object**，但这个 **object** 需要同时包含 _firstName_ 属性和 _lastName_ 属性，简单的说，利用 **&** 就是将多个对象类型进行 “合并” 成一个满足他们所有对象约束的新类型。

## 类型推断

如果在声明变量时没有添加类型约束，**TypeScript** 会根据关键字和给定值的类型自动进行类型推断（鼠标悬浮到变量名上会弹出对应提示）。

使用 _const_ 声明，赋值为字符串的值被推断为 “值类型”。
...
使用 _const_ 声明，赋值为对象的值被推断为包含 “相应属性” 的 “对象类型”。
...
在 **TypeScript** 中，“类型约束” 不是必须的，可以加，也可以不加，如果是一些基本类型如 **string**、**number** 往往可以利用 “类型推断” 的机制省略不写；但如果是一些非常复杂的数据类型，**TypeScript** 就不能保证可以一定可以推断出正确的数据类型了，因此，在大型项目的复杂场景下，显示的 “类型约束” 就显得极为重要了。
...
上面这段代码中，_firstName_ 和 _lastName_ 都被具体的 “值类型” 加以约束，意味着我只能赋值为这个具体的值，好比说写死了两个不能改变的 **常量**；而下面的 _fullName_ 通过 **string** 进行约束，意味着只要值的类型满足 **string** 类型，就可以赋值给它，因此，**string** 相比于 **值类型** 而言具有更广的范围，简单的说，就是 **string** 类型包含了 “值为字符串的值类型”，**string** 是 “值为字符串的值类型” 的父类型，反之，则为 “子类型”。
...
可以看到，子类型的值可以赋值给父类型，但是反过来就会报错，这个机制应该很好理解：

父类型 **string** 的范围更大，除了可以是 _Coder_ 或者 _Monkey_ 之外，也可以是其他一切类型为 **string** 的值，加上 “值类型” 的值又是固定不变的，你将一个不确定的 **string** 变量赋值给 **常量** 本身就是有问题的，所以，报错是必然的。
...
上面代码为 _user_ 添加了 “类型约束”，一个包含 _name_ 和 _age_ 的对象，试想一下，如果下次我又声明一个 _teacher_ 变量也想使用这个 “类型约束” 怎么办？要么重新写一份，要么拷贝过来，在这种情况下，就显得费时费力了，因此，可以将该类型抽离出来，单独定义一个新的别名，下次直接使用定义的别名类型即可。
...
通过 **type** 声明的 “对象类型”，在每一个 `key:value` 后面可以写 `,` 可以写 `;` 也可以 “留空不写”

为了和 “对象字面量” 的特点保持一致，在开发中，我个人更推荐在每一个 `key:value` 后面写上 `,` 最后一个 `key:value` 后面留空。

以下三种形式在 **TypeScript** 中都是合法的：
